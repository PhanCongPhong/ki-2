...
int teststd(char test[size]);        //compare which new idstd existed or not
void addstudent(char *Namefile);     //add new student
void deletestudent(char *Namefile);  //delete student
void editingStudent(char *filename); //edit information student
void sortingstudent(char *filename); //show all student
void findstudent(char *Namefile);    //find student
int readbook(char *Namefile);
void searchstudent(char *filestd, char *filebook, char *filelibrary); //Find books that students borrow
void mainstudent(char *login);                                        //menu student

//Part: Book

int testbook(char test[size]);    //compare which new ISBN book existed or not
void addbook(char *Namefile);     // add new book
void deletebook1(char *Namefile); //delete book
void kiemtra(char *searchISBN);    //When the nearest book will be delivered
void returnbook_student(char *filestd, char *filebook, char *filelibrary); //return book
void borrowbook_student(char *filestd, char *filebook, char *filelibrary); //search book
void searchbook(char *filestd, char *filebook, char *filelibrary);
void mainbook(char *login); //main book

//Part: User

int readuser(char *Namefile);
int testuser(char test[size]); //compare which new ISBN book existed or not
void adduser(char *Namefile);  //add new user
void ReadFile(char *Namefile); //main for login
...
void ReadFile(char *Namefile) // Main Menu for login
{
...
void mainstudent(char *login) //Menu student part1+3
{
    ...
    for (;;)
    {
        ...
        printf("7.Exit\n");
        scanf("Choose what you want 1,2,3,4, 5 ,6 or 7!!!\n");
        scanf("%d", &a);
        switch (a)
        {
        case 1:
...
void mainbook(char *login) //Menu book part2+3
{
    char filestd[max] = "student.csv";
    char filebook[max] = "book.csv";
    char filelibrary[max] = "student_books.csv";
...
void findstudent(char *Namefile)
{
    char deleteid[size];
    FILE *f;
    printf("Enter ID student to find\n");
    fflush(stdin);
    gets(deleteid);
    f = fopen(Namefile, "r");
    if (!f)
...
// --------------end-part1+part3-student-----------------//

//--------------- part2+part3-book-----------------------//

int testbook(char test[size]) //compare which new ISBN existed or not
{
...
void kiemtra(char *searchISBN)
{
    int date_min, month_min, year_min, k;
    int i = 0;
    int count = 0;
    struct library find[max];
    char temp[size];
    FILE *f;
    ...
    else
    {
        ...
        while (fgets(temp, size, f))
        {
            fscanf(f, "%[^,],%[^,],%d.%d.%d", lbr[i].ISBN, lbr[i].idstd, &lbr[i].date, &lbr[i].month, &lbr[i].year);
            i++;
        }
    }
    fclose(f);
    for (int j = 0; j < i; j++)
    {
        if (strcmp(searchISBN, lbr[j].ISBN) == 0)
        {
            find[count].date = lbr[j].date;
            find[count].month = lbr[j].month;
            find[count].year = lbr[j].year;
            count++;
        }
    }
    if (count == 0)
    ...
    {
        year_min = find[0].year;
        for (int j = 1; j < count; j++)
        {
            if (find[j].year < year_min)
            {
                year_min = find[j].year;
                month_min = find[j].month;
            }
        }
        for (int j = 0; j < count; j++)
        {
            if (find[j].year == year_min)
            {
                if (find[j].month < month_min)
                {
                    month_min = find[j].month;
                    date_min = find[j].date;
                }
            }
        }
        for (int j = 0; j < count; j++)
        ...
        for (int j = 0; j < count; j++)
        {
            if ((find[j].year == year_min) && (find[j].month == month_min) && (find[j].date == date_min))
            {

                printf("%d.%d.%d\n", find[j].date, find[j].month, find[j].year);
            }
        }
    }
}
...
void returnbook_student(char *filestd, char *filebook, char *filelibrary)
{
    ...
    else
    {
        ...
        for (t; t < n; t++)
        {
            ...
            int i = readbook("book.csv");
            int k = -1;
            int m = testbook(searchISBN);
            for (int j = 0; j < b; j++)
            {
                if ((strcmp(searchidstd, lbr[j].idstd) == 0) && (strcmp(searchISBN, lbr[j].ISBN) == 0))
                {
                    k = j;
                }
            }
            ...
            else if (book[m].access == book[m].totalbook)
            {
